...
void compute_intersections_1st(
    ...
    if (dimension == 0) { //Prepare intersections for mapping along X first (mapping order X-Y-Z)
        ...
        const Eigen::Matrix<Real,3,1> line_point(
            0.0,
            0.5*vmesh->getCellDx(1)+vmesh->getMeshMinLimits()[1],
            0.5*vmesh->getCellDx(2)+vmesh->getMeshMinLimits()[2]);
        const Eigen::Matrix<Real,3,1> lagrangian_di = bwd_transform.linear()
            * Eigen::Matrix<Real,3,1>(vmesh->getCellDx(0),0,0.0);
        const Eigen::Matrix<Real,3,1> euclidian_dj = Eigen::Matrix<Real,3,1>(0,vmesh->getCellDx(1),0.0);
        const Eigen::Matrix<Real,3,1> euclidian_dk = Eigen::Matrix<Real,3,1>(0.0,0.0,vmesh->getCellDx(2));
    ...
    if (dimension == 1) { //Prepare intersections for mapping along Y first (mapping order Y-Z-X)
        ...
        const Eigen::Matrix<Real,3,1> line_point(
            0.5*vmesh->getCellDx(0)+vmesh->getMeshMinLimits()[0],
            0.0,
            0.5*vmesh->getCellDx(2)+vmesh->getMeshMinLimits()[2]);  
        const Eigen::Matrix<Real,3,1> euclidian_di
            = Eigen::Matrix<Real,3,1>(vmesh->getCellDx(0), 0.0, 0.0); 
        ...
        const Eigen::Matrix<Real,3,1> euclidian_dk
            = Eigen::Matrix<Real,3,1>(0.0 , 0.0 ,vmesh->getCellDx(2)); 
    ...
    if (dimension == 2) {
        ...
        const Eigen::Matrix<Real,3,1> euclidian_dj
            = Eigen::Matrix<Real,3,1>(0,vmesh->getCellDx(1),0.0); 
        const Eigen::Matrix<Real,3,1> lagrangian_dk
            = bwd_transform.linear()*Eigen::Matrix<Real,3,1>(0.0,0.0,vmesh->getCellDx(2));
...
void compute_intersections_2nd(
    ...
    if (dimension == 0) { // Prepare intersections for mapping along X second (mapping order Z-X-Y)
        ...
        Eigen::Matrix<Real,3,1> plane_point
            = Eigen::Matrix<Real,3,1>(0,vmesh->getMeshMinLimits()[1]+vmesh->getCellDx(1)*0.5,0); 
        const Eigen::Matrix<Real,3,1> lagrangian_di
            = bwd_transform.linear()*Eigen::Matrix<Real,3,1>(vmesh->getCellDx(0),0,0.0);
    ...
    if (dimension == 1) { //Prepare intersections for mapping along Y second (mapping order X-Y-Z)
        ...
        Eigen::Matrix<Real,3,1> plane_point
            = Eigen::Matrix<Real,3,1>(0.0, 0.0,vmesh->getMeshMinLimits()[2]+vmesh->getCellDx(2) * 0.5);

        const Eigen::Matrix<Real,3,1> lagrangian_di
            = bwd_transform.linear() * Eigen::Matrix<Real,3,1>(vmesh->getCellDx(0), 0.0,  0.0); 
        const Eigen::Matrix<Real,3,1> lagrangian_dj
            = bwd_transform.linear() * Eigen::Matrix<Real,3,1>(0.0, vmesh->getCellDx(1), 0.0);
        // Distance between Euclidian planes
        const Eigen::Matrix<Real,3,1> euclidian_dk
    ...
    if (dimension == 2) { //Prepare intersections for mapping along Z second (mapping order Y-Z-X)
        ...
        Eigen::Matrix<Real,3,1> plane_point 
            = Eigen::Matrix<Real,3,1>(vmesh->getMeshMinLimits()[0]+vmesh->getCellDx(0)*0.5, 0.0, 0.0);
        // Distance between Euclidian planes
        const Eigen::Matrix<Real,3,1> euclidian_di = Eigen::Matrix<Real,3,1>(vmesh->getCellDx(0), 0.0, 0.0);
        const Eigen::Matrix<Real,3,1> lagrangian_dj 
            = bwd_transform.linear() * Eigen::Matrix<Real,3,1>(0.0, vmesh->getCellDx(1), 0.0); 
        const Eigen::Matrix<Real,3,1> lagrangian_dk = bwd_transform.linear() * Eigen::Matrix<Real,3,1>(0.0, 0.0, vmesh->getCellDx(2)); 
...
void compute_intersections_3rd(
    ...
    if (dimension == 0) { //Prepare intersections for mapping along X third (mapping order Y-Z-X)
        const Eigen::Matrix<Real,3,1> point_0_0_0 = bwd_transform
            * Eigen::Matrix<Real,3,1>(0.0 * vmesh->getCellDx(0) + vmesh->getMeshMinLimits()[0],
                                      0.5 * vmesh->getCellDx(1) + vmesh->getMeshMinLimits()[1],
                                      0.5 * vmesh->getCellDx(2) + vmesh->getMeshMinLimits()[2]);
        const Eigen::Matrix<Real,3,1> point_1_0_0 = bwd_transform
            * Eigen::Matrix<Real,3,1>(1.0 * vmesh->getCellDx(0) + vmesh->getMeshMinLimits()[0],
                                      0.5 * vmesh->getCellDx(1) + vmesh->getMeshMinLimits()[1],
                                      0.5 * vmesh->getCellDx(2) + vmesh->getMeshMinLimits()[2]);
        const Eigen::Matrix<Real,3,1> point_0_1_0 = bwd_transform
            * Eigen::Matrix<Real,3,1>(0.0 * vmesh->getCellDx(0) + vmesh->getMeshMinLimits()[0],
                                      1.5 * vmesh->getCellDx(1) + vmesh->getMeshMinLimits()[1],
                                      0.5 * vmesh->getCellDx(2) + vmesh->getMeshMinLimits()[2]);
        const Eigen::Matrix<Real,3,1> point_0_0_1 = bwd_transform
            * Eigen::Matrix<Real,3,1>(0.0 * vmesh->getCellDx(0) + vmesh->getMeshMinLimits()[0],
                                      0.5 * vmesh->getCellDx(1) + vmesh->getMeshMinLimits()[1],
                                      1.5 * vmesh->getCellDx(2) + vmesh->getMeshMinLimits()[2]);
   ...
   if (dimension == 1) { //Prepare intersections for mapping along Y third (mapping order Z-X-Y)
        ...
        // data along z has been moved to Lagrangian coordinates
        const Eigen::Matrix<Real,3,1> point_0_0_0 = bwd_transform
            * Eigen::Matrix<Real,3,1>(0.5 * vmesh->getCellDx(0) + vmesh->getMeshMinLimits()[0],
                                      vmesh->getMeshMinLimits()[1],
                                      0.5 * vmesh->getCellDx(2) + vmesh->getMeshMinLimits()[2]);
        const Eigen::Matrix<Real,3,1> point_1_0_0 = bwd_transform
        ...
        const Eigen::Matrix<Real,3,1> point_0_1_0 = bwd_transform
            * Eigen::Matrix<Real,3,1>(0.5 * vmesh->getCellDx(0) + vmesh->getMeshMinLimits()[0],
                                      1.0 * vmesh->getCellDx(1) + vmesh->getMeshMinLimits()[1],
                                      0.5 * vmesh->getCellDx(2) + vmesh->getMeshMinLimits()[2]);
        const Eigen::Matrix<Real,3,1> point_0_0_1 = bwd_transform
            * Eigen::Matrix<Real,3,1>(0.5 * vmesh->getCellDx(0) + vmesh->getMeshMinLimits()[0],
                                      vmesh->getMeshMinLimits()[1],
    ...
    if (dimension == 2) { //Prepare intersections for mapping along Z third (mapping order X-Y-Z)
        const Eigen::Matrix<Real,3,1> point_0_0_0 = bwd_transform
            * Eigen::Matrix<Real,3,1>(0.5 * vmesh->getCellDx(0) + vmesh->getMeshMinLimits()[0],
                                      0.5 * vmesh->getCellDx(1) + vmesh->getMeshMinLimits()[1],
                                      0.0 * vmesh->getCellDx(2) + vmesh->getMeshMinLimits()[2]);
        const Eigen::Matrix<Real,3,1> point_1_0_0 = bwd_transform
            * Eigen::Matrix<Real,3,1>(1.5 * vmesh->getCellDx(0) + vmesh->getMeshMinLimits()[0],
                                      0.5 * vmesh->getCellDx(1) + vmesh->getMeshMinLimits()[1],
                                      0.0 * vmesh->getCellDx(2) + vmesh->getMeshMinLimits()[2]);
        ...
        const Eigen::Matrix<Real,3,1> point_0_0_1 = bwd_transform
            * Eigen::Matrix<Real,3,1>(0.5 * vmesh->getCellDx(0) + vmesh->getMeshMinLimits()[0],
                                      0.5 * vmesh->getCellDx(1) + vmesh->getMeshMinLimits()[1],
                                      1.0 * vmesh->getCellDx(2) + vmesh->getMeshMinLimits()[2]);
        intersection = point_0_0_0[dimension];
        intersection_di = point_1_0_0[dimension]-point_0_0_0[dimension];
        intersection_dj = point_0_1_0[dimension]-point_0_0_0[dimension];
        intersection_dk = point_0_0_1[dimension]-point_0_0_0[dimension];
