...
inline void swapBlockIndices(velocity_block_indices_t &blockIndices, const uint dimension){
   swapIndices(blockIndices, dimension);
}
...
/*
Real to_lagrangian(Real z, Real intersection)
{
...
int to_indexK(Real z, Real intersection, Real gk_ratio, int dim, int meshID)
{
   ...
   return vmesh::getMeshWrapper()->at(meshID).getBlockIndex(z, dim, intersection, gk_ratio);
...
/*
Vec to_lagrangian(Real z, Vec intersection) {
   return z - intersection;
}
*/
...
Veci to_gk(Real z, Vec intersection, Real gk_ratio, int dim, int meshID)
{
   ...
   Veci rval = truncatei(intersection);
   for (uint32_t i = 0; i < intersection.size(); ++i) {
...
*/
   ...
   for(uint setIndex=0; setIndex< setColumnOffsets.size(); ++setIndex) {
      ...
      // TODO: Assumed here: intersection calculators are linear operator, so lagrangian coordinate is dependent on eulerian coordinate instead of index
      // LGTM?
      ...
      auto blockCoords {vmesh->getBlockCoordinates(blocks[columnBlockOffsets[setColumnOffsets[setIndex]]])};
      auto blockDv {vmesh->getCellSize(blocks[columnBlockOffsets[setColumnOffsets[setIndex]]])};
      auto vMin {vmesh->getMeshMinLimits()};
      swapIndices(blockCoords, dimension);
      swapIndices(blockDv, dimension);
      swapIndices(vMin, dimension);

      Realf intersection_di_global {(blockCoords[0] - vMin[0]) * intersection_di / blockDv[0]};
      Realf intersection_dj_global {(blockCoords[1] - vMin[1]) * intersection_dj / blockDv[1]};

      Realf max_intersectionMin = 0;
      max_intersectionMin =  std::max(max_intersectionMin, (WID - 1) * intersection_dj);
      max_intersectionMin =  std::max(max_intersectionMin, (WID - 1) * intersection_di);
      max_intersectionMin =  std::max(max_intersectionMin, (WID - 1) * (intersection_di + intersection_dj));
      max_intersectionMin += intersection + intersection_di_global + intersection_dj_global;

      Realf min_intersectionMin = 0;
      min_intersectionMin =  std::min(min_intersectionMin, (WID - 1) * intersection_dj);
      min_intersectionMin =  std::min(min_intersectionMin, (WID - 1) * intersection_di);
      ...
      for(uint columnIndex = setColumnOffsets[setIndex]; columnIndex < setColumnOffsets[setIndex] + setNumColumns[setIndex]; ++columnIndex){
         ...
         const Real firstBlock_dv {vmesh->getCellDx(cblocks[0], dimension)};
         const Real lastBlock_dv {vmesh->getCellDx(cblocks[n_cblocks - 1], dimension)};
         ...
         Realf lastBlockMaxV {lastBlockCoords[dimension] + WID * lastBlock_dv};
     ...
     /*now store pointer to blocks, cannot do it at the same time as adding
      ...
      for(uint columnIndex = setColumnOffsets[setIndex]; columnIndex < setColumnOffsets[setIndex] + setNumColumns[setIndex] ; columnIndex ++){
         ...
         // TODO is this stupid?
         std::vector<Realf> dvs ((n_cblocks + 2) * WID);
         for (int i = 0; i < n_cblocks; ++i) {
            const Real dv {vmesh->getCellDx(cblocks[i], dimension)};
            for (int j = 0; j < WID; ++j) {
               dvs[(i + 1)*WID + j] = dv;
            }
         }
         ...
         // Hideous
         auto indicesFirst {vmesh->getIndices(cblocks[0])};
         auto indicesLast {vmesh->getIndices(cblocks[n_cblocks - 1])};
         const Real dvFirst {vmesh::getMeshWrapper()->at(popID).getBlockDxFromIndex(indicesFirst[dimension] - 1, dimension) / WID};
         const Real dvLast {vmesh::getMeshWrapper()->at(popID).getBlockDxFromIndex(indicesLast[dimension] + 1, dimension) / WID};
         for (int j = 0; j < WID; ++j) {
            dvs[j] = dvFirst;
            dvs[(n_cblocks + 1) * WID + j] = dvLast;
         }
         ...
         */
         for (int j = 0; j < WID; j += VECL/WID){
            ...
            */
            ...
            auto firstBlockCoords {vmesh->getBlockCoordinates(cblocks[0])};
            auto firstBlockDv {vmesh->getCellSize(cblocks[0])};
            auto firstBlockIndex {vmesh->getIndices(cblocks[0])[dimension]};
            swapIndices(firstBlockCoords, dimension);
            swapIndices(firstBlockDv, dimension);

            Realf intersection_di_global {(firstBlockCoords[0] - vMin[0]) * intersection_di / firstBlockDv[0]};
            Realf intersection_dj_global {(firstBlockCoords[1] - vMin[1]) * intersection_dj / firstBlockDv[1]};

            // TODO We assume di/dj is constant in the vector. Looks like we only operate inside blocks, so I think that's true
            // Code is broken even when only accelerating in dy. So it's not this that's the biggest issue
            const Vec intersection_min = intersection + intersection_di_global + intersection_dj_global + to_realf(i_indices) * intersection_di + to_realf(j_indices) * intersection_dj;

            Real dv {firstBlockDv[2]};
            Real i_dv {1.0/dv};
...
#endif
*/
            const double gk_ratio {intersection_dk * i_dv};
            // std::cerr << "gk_ratio is " << gk_ratio << "\n";
            Vec v_r(firstBlockCoords[2]);
            Veci lagrangian_gk_r = to_gk(v_r, intersection_min, gk_ratio, dimension, popID);
            Vec k_r(firstBlockIndex * WID);   // TODO wtf is this actually?

            // std::cerr << "First old gk_r: " << truncatei((v_r-intersection_min)/intersection_dk)[0] << ", new gk_r: " << lagrangian_gk_r[0] << "\n";
            // std::cerr << "firstBlockIndex: " << firstBlockIndex << ", v_r: " << v_r[0] << ", k_r: " << k_r[0] << "\n";
            ...
            Vec lagrangian_v_r = to_lagrangian(v_r, intersection_min);
            ...
            for (uint k=0; k < WID * n_cblocks; ++k ){
               ...
               compute_ppm_coeff_nonuniform(dvs.data(), values + valuesColumnOffset + i_pcolumnv(j, 0, -1, n_cblocks), h4, k + WID, a, spatial_cell->getVelocityBlockMinValue(popID));
               #endif
               ...
               // I think...
               if (k % WID == 0) {
                  // This one can probably be refactored away
                  ...
                  // std::cerr << "Dim " << dimension << ", k " << k << ", intersection_dk " << intersection_dk << "\n";
               }
               ...
               Vec v_l = v_r;
               v_r += vmesh->getCellDx(cblocks[k/WID], dimension);
...
//#endif
               ...
               // std::cerr << "minGkIndex: " << minGkIndex << " = " << lagrangian_gk_l[minGkIndex] << ", columnMinBlockK: " << columnMinBlockK[columnIndex] << " = " << columnMinBlockK[columnIndex] * WID << "\n";
               // std::cerr << "maxGkIndex: " << maxGkIndex << " = " << lagrangian_gk_r[maxGkIndex] << ", columnMaxBlockK: " << columnMaxBlockK[columnIndex] << " = " << (columnMaxBlockK[columnIndex] + 1) * WID - 1 << "\n";
               ...
               for(int gk = minGk; gk <= maxGk; gk++){
                  ...
                  // std::cerr << "old v_norm_r: " << ((  min(  max( (gk + 1) * intersection_dk + intersection_min, v_l), v_r) - v_l) * i_dv)[0] << ", new: " << v_norm_r[0] << "\n";

                  // I think???
                  //const Vec euler_gk {to_double(gk - lagrangian_gk_r_first) * (dv / intersection_dk) + intersection_min * i_dv};
                  // const Vec euler_gk {(gk + 1) * gk_ratio + intersection_min * i_dv}; // idk what's up with this so don't ask
                  // const Vec v_norm_r = min(  max( euler_gk, k_l), k_r) - k_l;
