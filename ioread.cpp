...
bool readBlockData(
   ...
   for (uint popID=0; popID<getObjectWrapper().particleSpecies.size(); ++popID) {
      ...
      if(fileMeshBBox[0] != ourMeshParams.gridLength[0] ||
         ...
         const Real dVx = vmesh::getMeshWrapper()->velocityMeshes->at(meshID).getCellDx(0);
         ...
         const Real dVy = vmesh::getMeshWrapper()->velocityMeshes->at(meshID).getCellDx(1);
         ...
         const Real dVz = vmesh::getMeshWrapper()->velocityMeshes->at(meshID).getCellDx(2);
         for(const auto& c : fileVelCoordsY) {
