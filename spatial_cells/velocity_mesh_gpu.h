...
namespace vmesh {
   ...
   ARCH_HOSTDEV inline vmesh::GlobalID VelocityMesh::findBlock(vmesh::GlobalID cellIndices[3]) const {
      ...
      vmesh::GlobalID i_block = cellIndices[0] / mesh::getMeshWrapper()->at(meshID).blockLength[0];
      vmesh::GlobalID j_block = cellIndices[1] / mesh::getMeshWrapper()->at(meshID).blockLength[1];
      vmesh::GlobalID k_block = cellIndices[2] / mesh::getMeshWrapper()->at(meshID).blockLength[2];
   ...
   ARCH_HOSTDEV inline bool VelocityMesh::getBlockCoordinates(const vmesh::GlobalID globalID,Real coords[3]) const {
      ...
      coords[0] = mesh::getMeshWrapper()->at(meshID).meshMinLimits[0] + indices[0]*mesh::getMeshWrapper()->at(meshID).blockSize[0];
      coords[1] = mesh::getMeshWrapper()->at(meshID).meshMinLimits[1] + indices[1]*mesh::getMeshWrapper()->at(meshID).blockSize[1];
      coords[2] = mesh::getMeshWrapper()->at(meshID).meshMinLimits[2] + indices[2]*mesh::getMeshWrapper()->at(meshID).blockSize[2];
      return true;
   }
   ARCH_HOSTDEV inline void VelocityMesh::getBlockInfo(const vmesh::GlobalID globalID, Real* array) const {
      ...
      vmesh::LocalID indices[3];
      indices[0] = globalID % mesh::getMeshWrapper()->at(meshID).gridLength[0];
      indices[1] = (globalID / mesh::getMeshWrapper()->at(meshID).gridLength[0]) % mesh::getMeshWrapper()->at(meshID).gridLength[1];
      indices[2] = globalID / (mesh::getMeshWrapper()->at(meshID).gridLength[0] * mesh::getMeshWrapper()->at(meshID).gridLength[1]);
      ...
      array[0] = mesh::getMeshWrapper()->at(meshID).meshMinLimits[0] + indices[0]*mesh::getMeshWrapper()->at(meshID).blockSize[0];
      array[1] = mesh::getMeshWrapper()->at(meshID).meshMinLimits[1] + indices[1]*mesh::getMeshWrapper()->at(meshID).blockSize[1];
      array[2] = mesh::getMeshWrapper()->at(meshID).meshMinLimits[2] + indices[2]*mesh::getMeshWrapper()->at(meshID).blockSize[2];

      // Indices 3-5 contain the cell size.
      // The values are the same as if getCellSize(globalID,&(array[3])) was called
      array[3] = mesh::getMeshWrapper()->at(meshID).cellSize[0];
      array[4] = mesh::getMeshWrapper()->at(meshID).cellSize[1];
      array[5] = mesh::getMeshWrapper()->at(meshID).cellSize[2];
   }

   ARCH_HOSTDEV inline const Real* VelocityMesh::getBlockSize() const {
      return mesh::getMeshWrapper()->at(meshID).blockSize;
   }

   ARCH_HOSTDEV inline bool VelocityMesh::getBlockSize(const vmesh::GlobalID globalID, Real size[3]) const {
      size[0] = mesh::getMeshWrapper()->at(meshID).blockSize[0];
      size[1] = mesh::getMeshWrapper()->at(meshID).blockSize[1];
      size[2] = mesh::getMeshWrapper()->at(meshID).blockSize[2];
      return true;
   }

   ARCH_HOSTDEV inline const Real* VelocityMesh::getCellSize() const {
      return mesh::getMeshWrapper()->at(meshID).cellSize;
   }

   ARCH_HOSTDEV inline bool VelocityMesh::getCellSize(const vmesh::GlobalID globalID, Real size[3]) const {
      size[0] = mesh::getMeshWrapper()->at(meshID).cellSize[0];
      size[1] = mesh::getMeshWrapper()->at(meshID).cellSize[1];
      size[2] = mesh::getMeshWrapper()->at(meshID).cellSize[2];
      return true;
   }
   ...
   ARCH_HOSTDEV inline vmesh::GlobalID VelocityMesh::getGlobalID(const Real* coords) const {
      if (coords[0] < mesh::getMeshWrapper()->at(meshID).meshMinLimits[0] || coords[0] >= mesh::getMeshWrapper()->at(meshID).meshMaxLimits[0] ||
         (coords[1] < mesh::getMeshWrapper()->at(meshID).meshMinLimits[1] || coords[1] >= mesh::getMeshWrapper()->at(meshID).meshMaxLimits[1] ||
          coords[2] < mesh::getMeshWrapper()->at(meshID).meshMinLimits[2] || coords[2] >= mesh::getMeshWrapper()->at(meshID).meshMaxLimits[2])) {
         return invalidGlobalID();
      }

      const vmesh::LocalID indices[3] = {
         static_cast<vmesh::LocalID>(floor((coords[0] - mesh::getMeshWrapper()->at(meshID).meshMinLimits[0]) / mesh::getMeshWrapper()->at(meshID).blockSize[0])),
         static_cast<vmesh::LocalID>(floor((coords[1] - mesh::getMeshWrapper()->at(meshID).meshMinLimits[1]) / mesh::getMeshWrapper()->at(meshID).blockSize[1])),
         static_cast<vmesh::LocalID>(floor((coords[2] - mesh::getMeshWrapper()->at(meshID).meshMinLimits[2]) / mesh::getMeshWrapper()->at(meshID).blockSize[2]))
      };

      return indices[2]*mesh::getMeshWrapper()->at(meshID).gridLength[1]*mesh::getMeshWrapper()->at(meshID).gridLength[0]
              + indices[1]*mesh::getMeshWrapper()->at(meshID).gridLength[0] + indices[0];
   }

   ARCH_HOSTDEV inline vmesh::GlobalID VelocityMesh::getGlobalID(const vmesh::LocalID indices[3]) const {
      if (indices[0] >= mesh::getMeshWrapper()->at(meshID).gridLength[0]) {
         return invalidGlobalID();
      }
      if (indices[1] >= mesh::getMeshWrapper()->at(meshID).gridLength[1]) {
         return invalidGlobalID();
      }
      if (indices[2] >= mesh::getMeshWrapper()->at(meshID).gridLength[2]) {
         return invalidGlobalID();
      }
      return indices[0] + indices[1] * mesh::getMeshWrapper()->at(meshID).gridLength[0]
         + indices[2] * mesh::getMeshWrapper()->at(meshID).gridLength[1]
         * mesh::getMeshWrapper()->at(meshID).gridLength[0];
   }
   ...
   ARCH_HOSTDEV inline vmesh::GlobalID VelocityMesh::getGlobalID(const vmesh::LocalID i,const vmesh::LocalID j,const vmesh::LocalID k) const {
      if (i >= mesh::getMeshWrapper()->at(meshID).gridLength[0]) {
         return invalidGlobalID();
      }
      if (j >= mesh::getMeshWrapper()->at(meshID).gridLength[1]) {
         return invalidGlobalID();
      }
      if (k >= mesh::getMeshWrapper()->at(meshID).gridLength[2]) {
         return invalidGlobalID();
      }
      return i + j * mesh::getMeshWrapper()->at(meshID).gridLength[0]
         + k * mesh::getMeshWrapper()->at(meshID).gridLength[1]
         * mesh::getMeshWrapper()->at(meshID).gridLength[0];
   }
   ...
   ARCH_HOSTDEV inline const vmesh::LocalID* VelocityMesh::getGridLength() const {
      return mesh::getMeshWrapper()->at(meshID).gridLength;
   }
   ...
   ARCH_HOSTDEV inline void VelocityMesh::getIndices(const vmesh::GlobalID globalID,vmesh::LocalID& i,vmesh::LocalID& j,vmesh::LocalID& k) const {
      if (globalID >= invalidGlobalID()) {
         ...
         i = globalID % mesh::getMeshWrapper()->at(meshID).gridLength[0];
         j = (globalID / mesh::getMeshWrapper()->at(meshID).gridLength[0]) % mesh::getMeshWrapper()->at(meshID).gridLength[1];
         k = globalID / (mesh::getMeshWrapper()->at(meshID).gridLength[0] * mesh::getMeshWrapper()->at(meshID).gridLength[1]);
      }
   }
   ...
   ARCH_HOSTDEV inline const Real* VelocityMesh::getMeshMaxLimits() const {
      return mesh::getMeshWrapper()->at(meshID).meshMaxLimits;
   }

   ARCH_HOSTDEV inline const Real* VelocityMesh::getMeshMinLimits() const {
      return mesh::getMeshWrapper()->at(meshID).meshMinLimits;
   }
   ...
   ARCH_HOSTDEV inline bool VelocityMesh::isInitialized() const {
      return mesh::getMeshWrapper()->at(meshID).initialized;
   }
   ...
   ARCH_HOSTDEV inline bool VelocityMesh::push_back(const vmesh::GlobalID globalID) {
      const size_t mySize = size();
      if (mySize >= mesh::getMeshWrapper()->at(meshID).max_velocity_blocks) {
         return false;
      }
      if (globalID == invalidGlobalID()) {
         return false;
   ...
   inline vmesh::LocalID VelocityMesh::push_back(const std::vector<vmesh::GlobalID>& blocks) {
      ...
      if (ltg_size+blocksSize > mesh::getMeshWrapper()->at(meshID).max_velocity_blocks) {
   ...
   **/
   ...
   ARCH_DEV inline vmesh::LocalID VelocityMesh::warpPush_back(const split::SplitVector<vmesh::GlobalID>& blocks, const size_t b_tid) {
      ...
      if (mySize+blocksSize > mesh::getMeshWrapper()->at(meshID).max_velocity_blocks) {
         if (b_tid==0) {
